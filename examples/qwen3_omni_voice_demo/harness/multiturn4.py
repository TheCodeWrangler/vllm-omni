...
TURNS=[("Madrid","q_raw.wav"),("Portland","q_pdx.wav"),("Tokyo","q_tokyo.wav"),
       ("Denver","q_denver.wav"),("__RECALL__","q_follow.wav")]
...
async def main():
    ...
    for i,(city,wav) in enumerate(TURNS, start=1):
        ...
        if city=="__RECALL__":
            # "which city did I just ask about?" -> should recall Denver, no tool needed
            ok = "denver" in text.lower()
            print(f"{i:>4} {'RECALL':>9} {nrep:>9} {str(called):>26}  "
                  f"{'OK' if ok else 'FAIL'}  {text[:70]!r}")
            if not ok: fails+=1
            continue
